...
class FlightControllerParser(FoxeerParser):
    URL = 'https://www.foxeer.com/flight-controller-t-78'

    def __init__(self):
        super().__init__(self.URL)

    def parse_page(self):
        FlightControllers = self.soup.find_all('li', class_='product-item col col-md-4 col-6')
        page_data = []

        for FlightController in FlightControllers:
            link = FlightController.find('a')['href']
            product_soup = self.get_soup(link)
            images = self.get_product_images(product_soup)
            name = FlightController.find('h3', class_='product-name product_title')
