...
class VtxParser(FoxeerParser):
    URL = 'https://www.foxeer.com/vtx-vrx-t-80'

    def __init__(self):
        super().__init__(self.URL)

    def parse_page(self):
        Vtxs = self.soup.find_all('li', class_='product-item col col-md-4 col-6')
        page_data = []

        for Vtx in Vtxs:
            link = Vtx.find('a')['href']
            product_soup = self.get_soup(link)
            images = self.get_product_images(product_soup)
            name = Vtx.find('h3', class_='product-name product_title')
